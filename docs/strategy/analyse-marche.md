...
- Différenciateurs à explorer : **offre packagée assurance RC syndic bénévole** (candidats : MMA, assureurs affinitaires) — potentiellement l'argument tueur du segment, **mais 100 % non validé** : hypothèse sourcée d'aucune donnée, elle porte beaucoup de poids narratif dans les trois documents. À confirmer en priorité dans les interviews *avant* d'en faire l'axe central — et un partenariat assurance est réplicable par un concurrent (Vilogi, un assureur qui descend en marque blanche), donc différenciateur d'exécution, pas douve durable. Par ailleurs : onboarding assisté (téléphonique) plutôt que « 5 minutes sans carte bancaire », et UX testée avec des 65+.
...
**Unit economics — le trou du document, assumé.** Ni CAC chiffré, ni LTV, ni churn, ni coût de support ne sont modélisés à ce jour. Premier ordre de grandeur à confronter aux données réelles : au churn de 3,5 %/mois retenu par le doc 3/3 et à un ARPU ~24 € (mix réaliste quasi-tout Essentiel), la **LTV brute ≈ 690 €** (~480 € en marge [hypothèse]) — soit un LTV/CAC ~2:1 en régime et négatif à l'amorçage. **À ce churn, l'économie unitaire est structurellement mince quel que soit le CAC** ; les deux seuls leviers sont de faire baisser le churn (rattacher le compte à la copropriété, pas au bénévole) et de monter l'ARPU via P3. Points durs : (a) des milliers de comptes gratuits = des bénévoles non techniques avec des questions *juridiques et comptables* — coût de support à modéliser (self-service + communauté, sinon le freemium est insoutenable) [hypothèse] ; (b) le churn bénévole a une cause structurelle : **des copropriétés repassent en syndic professionnel** (taux inconnu) ; (c) à 19 €/mois, un payback < 12 mois exige un CAC quasi nul, donc de l'organique pur ([Bantrr](https://bantrr.com/business-model/saas-metrics/cac-payback-benchmarks-for-saas-companies/)).

**Et la question qui domine toutes les autres : le socle produit à construire coûte plus cher que le revenu qu'il génère à 3 ans.** Ouvrir l'expansion d'ARPU (P3) suppose reprise de gestion + LRE + moteur décret 2005-240 (2-3 trimestres) + comptes séparés + RGPD — ~4-6 trimestres-ingénieur en domaine régulé, pour un revenu central de ~15-30 M€ de TAM mais seulement **100-250 K€ d'ARR à 3 ans** (§2.3). Le build pro excède plausiblement trois ans de revenu beachhead : c'est un arbitrage **bootstrap discipliné (rester bénévole, ne bâtir le socle pro qu'une fois financé par le cash-flow) vs levée (financer le socle, mais alors documenter le TAM/expansion venture)** à trancher *avant* d'engager le build. Décision : **P&L à 3 ans (churn, coût du gratuit, coût du build, payback) et choix de trajectoire de financement, avant toute présentation investisseurs et avant tout démarrage du socle pro.**
...
| **Désintermédiation par les associations** : l'ARC/l'ANCC recommandent ou labellisent *leur propre* outil (ou celui d'un concurrent) — le partenariat-moat a un revers, une organisation qui peut prescrire peut aussi prescrire ailleurs | Moyenne | Élevé | Ne pas faire des associations la *seule* douve ; nouer le lien tôt et le rendre mutuellement dépendant (contenu co-brandé, données), diversifier les canaux (expert-comptable, comparateurs, SEO propriétaire) |
